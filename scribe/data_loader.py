...
class DataLoader:
    ...
    def __init__(self, res, chrom, start, end, size):
        ...
        self.unmappable_inds = []
        self.unmappable_method = "drop"
    ...
    def load_hic(self, filename, KR=True, unmappable_method=None, rescale_method="ones"):
        ...
        Args:
            ...
            unmappable_method: How to handle unmappable bins (zero diagonal).
                "drop" (default): remove rows/columns.
                "impute": fill with average P(s) contact probability.
            rescale_method: Method to rescale contact map ("mean" or "ones")

        Returns:
            Contact map of shape (self.size, self.size)
        """
        if unmappable_method is None:
            unmappable_method = self.unmappable_method
        ...
        # Replace NaN with 0 (KR normalization can produce NaN)
        contactmap = np.nan_to_num(contactmap, nan=0.0)

        self.bigsize, _ = np.shape(
            contactmap
        )  # used for loading the correct number of chipseq bins

        # Identify unmappable bins (zero diagonal)
        unmappable_mask = np.diagonal(contactmap) == 0
        self.unmappable_inds = np.where(unmappable_mask)[0].tolist()

        if unmappable_method == "drop":
            contactmap, _ = analysis.clean_contactmap(contactmap)
        elif unmappable_method == "impute":
            ps = hiclib.compute_ps(contactmap, unmappable_mask)
            contactmap = hiclib.impute_unmappable(contactmap, unmappable_mask, ps)
        else:
            raise ValueError(
                f"unmappable_method must be 'drop' or 'impute', got '{unmappable_method}'"
            )
    ...
    def load_hic_pooled(
        ...
        Args:
            ...
            KR: Use Knight-Ruiz normalization
            unmappable_method: How to handle unmappable bins.
                "drop" (default): remove rows/columns.
                "impute": fill with average P(s) contact probability.
            rescale_method: Method to rescale contact map ("mean" or "ones")
        ...
        if cache and cache_file.exists():
            ...
            gthic = highres_loader.load_hic(
                filename, KR=KR, unmappable_method=unmappable_method, rescale_method=rescale_method
            )

            # Store unmappable indices for ChIP-seq loading compatibility
            self.unmappable_inds = highres_loader.unmappable_inds
    ...
    def load_bigWig(self, filename, method="mean"):
        ...
        if self.unmappable_method == "impute":
            # Zero out unmappable positions instead of deleting
            signal[self.unmappable_inds] = 0
        else:
            # Drop unmappable positions (original behavior)
            signal = np.delete(signal, self.unmappable_inds)

        return signal[0 : self.size]
