...
from scribe import analysis, energy_utils, utils
...
def sim_analysis(sim):
    ...
    Args:
        sim: Simulation object (analysis.SimulationResult or scribe_sim.ScribeSim)
...
def compare_analysis(sim):
    ...
    """
    plt.figure()
    analysis.plot_tri(sim.hic, sim.gthic, oe=True)
...
def maxent_analysis(sim):
    ...
    Outputs:
        ...
        Saves: ../SCC.png
    """
    SCC = analysis.get_SCC(sim.hic, sim.gthic)
    RMSE = analysis.get_RMSE(sim.hic, sim.gthic)
    RMSLE = analysis.get_RMSLE(sim.hic, sim.gthic)
...
def main():
    sim = analysis.SimulationResult("production_out")
