...
public:
	...
	std::vector<Cell*> active_cells_vec;          // same cells as active_cells, as a flat vector for allocation-free energy loops
	...
	double densityCapEnergy(const std::vector<Cell*>& flagged_cells);
	double energy(const std::vector<Cell*>& flagged_cells, const Eigen::MatrixXd &chis);
	double constantEnergy(const std::vector<Cell*>& flagged_cells, const double constant_chi);
	double diagEnergy(const std::vector<Cell*>& flagged_cells, const std::vector<double> &diag_chis);
	double boundaryEnergy(const std::vector<Cell*>& flagged_cells, const double boundary_chi);
	// double boundaryEnergy(const std::vector<Cell*>& flagged_cells, const std::vector<std::vector<double>> &Smatrix);
