...
pytestmark = pytest.mark.skipif(
    not HAS_SIMULATION,
    reason="pyticg C++ extension not built (run `make all`)"
)
...
class TestIdealChainSimulation:
    ...
    def test_ideal_chain_runs(self, temp_output_dir):
        ...
        assert (temp_output_dir / "test_run" / "hic.npy").exists() or \
               os.path.exists(os.path.join(temp_output_dir, "test_run", "hic.npy"))
